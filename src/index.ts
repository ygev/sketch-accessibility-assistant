    ...
    for (const group of utils.objects.group) {
      if (group.layers.length == 2) {
        if (group.layers[0]._class == 'text' && group.layers[1]._class == 'text') {
          ...
          utils.report(
            `Here are the layers:“${group.name}. Layer0 Color: ${group.layers[0].style?.fills?.[0]?.color?.red}, Layer1 Color: ${group.layers[1].style?.textStyle?.encodedAttributes.MSAttributedStringColorAttribute?.red}”`,
            group,
          )
        }
      }
