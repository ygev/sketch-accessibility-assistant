...
function calcAAAContrastText(sketchObject: Context, utils: Context) {
        ...
        for (var j = 1; j < maxLayers; j++) {
          ...
          if (topXmin < botXmax && botXmin < topXmax && topYmin < botYmax && botYmin < topYmax) {
            ...
            if (ratio >= 4.5 && txtSize >= 18) {
              ...
              utils.report(`Layers ${sketchObject.layers[i].name} and ${sketchObject.layers[j].name} do not pass WCAG 2.1 AAA, and their contrast ratio is: ${ratio.toFixed(2)}:1`, sketchObject.layers[j])
            }
          }
        }
      }
...
export const AAAContrastText: RuleDefinition = {
  rule: async (context) => {
    ...
    for (const artboard of utils.objects.artboard) {
      calcAAAContrastText(artboard, utils)
    }
    for (const group of utils.objects.group) {
      calcAAAContrastText(group, utils)
    }
  },
