...
export const AAAContrastOverlap: RuleDefinition = {
  rule: async (context) => {
    const { utils } = context
        ...
        for (var j = i; j < maxLayers; j++) {
          ...
          if (topXmin < botXmax && botXmin < topXmax && topYmin < botYmax && botYmin < topYmax) {
            ...
            if (ratio >= 4.5 && txtSize >= 18) {
              ...
              utils.report(`These shape elements do not pass WCAG 2.1 AAA, and their contrast ratio is: ${ratio.toFixed(2)}:1`, artboard.layers[j])
            }
          }
        }
      }
